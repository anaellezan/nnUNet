''' This script aims at:
- resampling labels into the space (direction/spacing) of predictions (same as reference)
- computing opacification defect mask and saving it.
The opacification defect is created from the logical operation of (the union between the LA prediction and the auricle label) minus the prediction.
'''
...
def main():
    ...
    pred_files = [
        f'/media/sharedata/atriumCT/atrium_nnunet/raw_data/{dataset}/prediction_3d_dataset002/'
        + f
        for f in files
    ]
    ...
    for i in tqdm(range(len(pred_files))):
        ...
        ref_label_file = (
            f'/media/sharedata/atriumCT/corrected_data/GTlabels/{origin_id}.mha'
        )
        ...
        seg_ref_label_to_seg_pred_arr = sitk.GetArrayFromImage(
            seg_ref_label_to_seg_pred
        )

        label_reoriented_path = f'/media/sharedata/atriumCT/corrected_data/GTlabels_reoriented/{origin_id}.mha'
        utils.test_empty_arr(
            seg_ref_label_to_seg_pred_arr, label_reoriented_path, 1
        )
        utils.test_unique_val(
            seg_ref_label_to_seg_pred_arr, label_reoriented_path, 7
        )

        # Save reoriented labels
        sitk.WriteImage(
            seg_ref_label_to_seg_pred,
            label_reoriented_path,
            useCompression=True,
        )
        ...
        opacification_defect_mask = sitk.GetImageFromArray(
            opacification_defect_arr
        )
        opacification_defect_mask = sitk.Cast(
            opacification_defect_mask, sitk.sitkUInt8
        ...
        utils.test_empty_arr(
            opacification_defect_arr, opacification_defect_mask_path, 0.003
        )
        # Save opacification defect masks
        sitk.WriteImage(
            opacification_defect_mask,
            opacification_defect_mask_path,
            useCompression=True,
        )
