...
ref_files = [f'/media/sharedata/atriumCT/atrium_nnunet/raw_data/Dataset004_LA_CT00_corrected_voted/labelsTr/' + f.replace('la_lables_opa_defect', 'la_trans_corrected').replace('_0.mha','.mha') for f in files]
print(len(ref_files))


# ref_label_files = ['/media/sharedata/atriumCT/corrected_data/GTlabels/' + f[-11:-8].split('_')[-1] + '.mha' for f in files]
ref_label_files = ['/media/sharedata/atriumCT/atrium_nnunet/raw_data/Dataset005_LA_CT00_corrected_voted_region/labelsTr/' + f.replace('_0.mha','.mha') for f in files]
...
def main():
    ...
    for i in tqdm(range(len(pred_files))):
        ...
        seg_ref_auricule_arr = ((seg_ref_label_arr == 2)|(seg_ref_label_arr == 7))
        ...
        if len(np.unique(seg_ref_auricule_arr)) > 1 :
            ...
            contour_volume_auricle = metrics.compute_contour_volume(multiplication_bbox_auricle_seg_pred, multiplication_bbox_auricle_seg_ref)
        ...
        df.loc[df['ID']==origin_id,'Contour_vol_auricle'] = contour_volume_auricle
        ...
        df.loc[df['ID']==origin_id,'vol_auricle_ref'] = seg_ref_auricule_arr.sum()
        df.loc[df['ID']==origin_id,'vol_opa_ref'] = seg_ref_opa_defect_arr.sum()
        df.loc[df['ID']==origin_id,'vol_opa_pred'] = seg_pred_opa_defect_arr.sum()
        print(df)
