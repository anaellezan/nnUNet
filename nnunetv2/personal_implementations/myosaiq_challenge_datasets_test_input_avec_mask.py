...
if __name__ == "__main__":
    ...
    dataset_name = 'Dataset016_D8CT_mask'
    ...
    for fold in range(FOLDS_NUMBER):
      source_pred_dir = f'/beegfs/azanella/data_challenge/nnUNet/nnUNet_results/Dataset010_D8/nnUNetTrainer__nnUNetPlans__3d_fullres/fold_{fold}/validation/'
    ...
    generate_dataset_json(output_folder=join(nnunet_dir, dataset_name),
                          channel_names={0: 'CT',
                                        1: 'MRI'}, # seg
                          labels={'background': 0,  'total_infarct': [1,2], 'mvo':[2] },
                          num_training_cases=len(source_images),
                          file_ending='.nii.gz',
                          dataset_name=dataset_name,
                          regions_class_order=[1, 2], # ['total_infarct', 'mvo']
                          )
